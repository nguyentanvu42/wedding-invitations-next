...
export async function getWishes() {
  const { rows } = await getPool().query(
    'SELECT * FROM wishes ORDER BY created_at DESC LIMIT 200'
  )
  return rows.map(w => ({
    id: w.id,
    guestId: w.guest_id,
...
export async function countRecentWishes({ guestId, ip, seconds }) {
  const col = guestId ? 'guest_id' : 'ip'
  const val = guestId || ip
  if (!val) return 0
  const { rows } = await getPool().query(
...
export async function createWish({ guestId, guestName, message, ip = null }) {
  const { rows } = await getPool().query(
    `INSERT INTO wishes (guest_id, guest_name, message, ip)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
    [guestId, guestName, message, ip]
  )
  const w = rows[0]
  return {
    id: w.id,
